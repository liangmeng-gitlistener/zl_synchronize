...
import cn.cncommdata.enums.TaskStatusEnum;
...
public class ScheduleConfig {
     ...
     * 动态设置定时任务方法
     * @param cronTriggers
     * 此方法是真正的动态实现启停和时间周期的关键，你可以针对自己的业务来调用，你对库中的动态数据修改后来调用此方法，每个Cron对象必须要包含，执行周期（cron.getCron()），
     * 启停状态（cron.getCronStatus()），执行的类（cron.getCronClass()）
     */
    public List<TCronTrigger> startCron(List<TCronTrigger> cronTriggers) {
        try {
            if (ObjectUtil.isNotEmpty(cronTriggers)) {
                ...
                for (TCronTrigger cron : cronTriggers) {
                    //判断当前定时任务是否有效
                    if (cron.getIsDeleted().equals(TaskStatusEnum.RUNNING_TASK.getState())) {
                        //  开启一个新的任务，库中存储的是全类名（包名加类名）通过反射成java类，读取新的时间
                        //  等价于(Runnable) Class.forName(cron.getClassName()).newInstance()
                        Runnable runnable = (Runnable) ReflectUtil.newInstance(ClassLoaderUtil.loadClass(cron.getClassName()));
                        ScheduledFuture<?> future = threadPoolTaskScheduler.schedule(runnable, getTrigger(cron));
